...
date: "Material for Lab 7 of Landscape Analysis and Modeling, Spring 2016"
output: pdf_document
---

*This document, with active hyperlinks, is available online at:[http://mltconsecol.github.io/TU_
LandscapeAnalysis_Documents/Assignments_web/Assignment07_Mantel_SpRegress.html](http://mltconsecol.github.io/TU_
LandscapeAnalysis_Documents/Assignments_web/Assignment07_Mantel_SpRegress.html)*

**Due Date: Thursday, 22 March 2016**
...
Though it is informative to identify whether there is spatial autocorrelation for individual variables (e.g., with Moran's *I* or Geary's *c*), as we did in [Lab 6](./Assignment06_Autocorrelation.html), in ecology and evolutionary biology, we are often interested in the relationships of multiple variables. For example, how does temperature affect tree size or the ecological community of species at a site? As we discussed in lecture, the effect of spatial autocorrelation can obscure actual patterns in the data, thus we must deal with this appropriately.

We will focus this lab on an example dataset from Chapter 7 of the book *Mixed Effects Models and Extensions in Ecology with R* by Alain Zuur et al, as part of [Highland Statistics Ltd.](http://www.highstat.com).  You can check out the book's website, here: [http://www.highstat.com/book2.htm](http://www.highstat.com/book2.htm). This group has a few books on analyzing ecological data with R - I recommend looking into their books for additional methods you may use in your research, and definitely check out their published papers too.

You can download [data](http://www.highstat.com/Book2/ZuurDataMixedModelling.zip) used in the book at the book's website. We will focus on the "Boreality" dataset (Boreality.txt). You can also download all [R code](http://www.highstat.com/Book2/ZuurMixedModellingAllRCode.zip) used in the book, but everything you need for this exercise is provided herein.
...
Though we have discussed the problems with Mantel tests in lecture, and they are well documented in the literature (e.g., [Guillot and Rousset 2014](http://onlinelibrary.wiley.com/doi/10.1111/2041-210x.12018/abstract)), you may wish to explore the analyses with data. These these methods are still used frequently in the literature, and may give a quick idea of what may be going on in a focal system.
...
The output for each will look something like what is pasted below (the result is from 'mantel(bor.dist~geog.dist)'. The 'mantelr' value is the calculated mantel correlation between the two matrices; the first two p-values are for one-way tests (testing wither r is <= 0 or >= 0, respectively), while the third is for a two-way test (testing whether r is significantly different from expected in either direction) - generally I suggest focusing on that third one (pval3) unless your hypothesis is specifically that there is a positive or negative relationship. The 'llim.2.5%' and 'ulim.97.5%' show the 95% confidence interval for the Mantel's r based on bootstrap resampling. This effectively provides an estimate of the potential values for r for a population, given that we typically only have a sample of data. These results show there is small but significant correlation (Mantel's r = 0.047, p=0.001) between the boreality values and the geographic distance between samples.

      mantelr      pval1      pval2      pval3  llim.2.5% ulim.97.5% 
    0.04712141 0.00100000 1.00000000 0.00100000 0.03555909 0.05971639 
...
![](../Assignment_Images/Lab7_ResidBubblePlot.png)\

As you can see, there appears to be clustering of high and low residual values. 

We can visualize spatial autocorrelation in the residuals as we did last week using the 'pgirmess' package and the function 'correlog' (Look back to [Lab 6](./Lab6_Autocorrelation.Rmd))
...
![](../Assignment_Images/Lab7_lmResidVar.png)\
...
![](../Assignment_Images/Lab7_b1glsVario.png)\

To add the spatial correlation structure to the error, we can use add the 'correlation' argument to the nlme function. Herein, we can define the type of variogram model to use, indicate whether to use a nugget. To view the options, you can look up help for 'corClasses', and specifications for each option you can use help for each too. The main ones used for spatial autocorrelation are: corExp, corGaus, corLin, corRatio, and corSpher.
