...
**Answer the 10 questions at end of document, worth 20 points total (2 pts each). The first 6 questions are focused on quadrat variance analyses; the other 4 involve working with your own data in R.**
...
You can download the dataset, store it locally and load it into R, as you did in the [previous lab](./Lab3_Import_Regress_RipleysK.Rmd). Alternatively, for datasets stored online, you can actually import them directly by using the appropriate import command and using the url of the dataset. For this lab we'll import the data and assign it to 'HTrees'. It is good practice to set your working directory to your desired folder, so do that too.
