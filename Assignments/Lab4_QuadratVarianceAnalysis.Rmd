...
**Answer the 10 questions at end of document, worth 20 points total. The first 6 questions are focused on quadrat variance analyses; the other 4 involve workign with your own data in R.**
...
Now that we have the quadrats set up with counts for our given datasets, we can run quadrat variance analyses. Click on the 'Analysis' tab (not 'Analysis 2'), then click on the 'QuadVar' icon, and select 'Quadrat Variance Methods'.


![](./Images/Passage_QuadVar.png)\



There are a few things you'll want to specify here - though I describe some of them briefly for this lab, I suggest looking at the Help - if nothing else, take a quick look to see the kind of useful details it gives.

You set things up as illustrated in the following image. 


![](./Images/Passage_QuadVarWindow.png)\
...
7) Import your data into R - show that you've done this by copying and pasting either the 'head' or 'structure' of your dataset.

8) Make a histogram of a continuous variable that you have.

9) Plot two variables against one another, as you've done for previous assignments. If you only have one continuous variable and the other is a factor variable, present boxplots - one for each group.

10) If you have a question you are looking to answer with these data, describe it here in a sentence or two.